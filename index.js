...
const pg = require('pg');
const { Client, Collection, Intents, MessageAttachment, DiscordAPIError } = require('discord.js');
...
client.getChannel = function (guildId) {
  dbClient.query(`SELECT channel_id FROM public.settings WHERE guild_id = '${guildId}'`, (err, res) => {
    if (err) {
      console.log("An error has occured in getting the channel ID of a guild!");
      console.log(err);
      return null;
    }
...
client.once('ready', () => {
    ...
    // setup db
    dbClient.query("SELECT EXISTS ( SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'settings' );", (err, res) => {
      if (err) {
    ...
    // wednesday checker
    const itIsWednesday = Cron('00,01 00 00 * * 3', { timezone: "Asia/Manila" } , () => {
