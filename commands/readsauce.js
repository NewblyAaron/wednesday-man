...
async function fetchPage(interaction, sauceCode, pageNum, ephemeral_value) {
    nhentai_api.getBook(sauceCode).then(async book => {
        ...
        const Page = new MessageAttachment(url);
        await interaction.editReply({ content: `Reading **${book.title.pretty}** | ${sauceCode}\nPage: ${pageNum + 1}/${book.pages.length}`, components: [row], files: [Page], ephemeral: true });
    })
    .catch(async err => {
        console.log(err);
        
        await interaction.editReply({ content: `Error has occurred.`, components: [], files: [], ephemeral: ephemeral_value });
...
module.exports = {
	...
	async execute(interaction) {
        ...
        var pageNum = 0, pageMax = (await nhentai_api.getBook(sauceCode)).pages.length;
        ...
        const collector = interaction.channel.createMessageComponentCollector({ componentType: 'BUTTON', idle: 60000 });

        collector.on('collect', async i => {
            if (!(interaction.user.id === i.user.id))
                return i.reply({ content: `Only ${interaction.user} can interact with the buttons!`, ephemeral: true });
        ...
        collector.on('end', async collected => {
            await interaction.editReply({ content: `No activity detected after a minute or stopped.`, components: [], files: [], ephemeral: true });
        ...
        await interaction.reply({ content: `Controller: ${interaction.user}\nReading ${sauceCode}\n**WARNING:** Only run one \`/runsauce\` command at a time! Feel free to use the **Stop reading** button to read another book.\nLastly, if no interactions have happened in a minute, it will automatically stop.`, components: [row], ephemeral: ephemeral_value });
	},
};